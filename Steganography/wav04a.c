// wav04a.c - Complement LSB of each byte of the audio file using exclusive-OR (^)
...
// Take a pair of bytes and complement its LSB, then copy such transformed bytes to output file
	odd = 1;
	for (i = 0; i < 10; i++) {
		c = getc(ifp);
		if (odd == 0) {
